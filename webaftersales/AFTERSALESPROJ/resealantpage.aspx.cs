...
{
    public partial class resealantpage : System.Web.UI.Page
    {
        ...
        private void loaddata()
        {
            try
            {
                ...
                using (SqlConnection sqlcon = new SqlConnection(sqlconstr))
                {
                    ...
                    string str = "select [ID],[CIN],[SID],[QNO],qdate,[PREPAREDBY],[NOTEDBY] from Re_Sealant_Quotation_Tbl where cin=@cin and sid=@sid";
            ...
            if (IsValid)
            {
                try
                {
                    ...
                    using (SqlConnection sqlcon = new SqlConnection(sqlconstr))
                    {
                        ...
                        string str = "declare @id as integer = (select isnull(max(isnull(id,0)),0)+1 from [Re_Sealant_Quotation_Tbl]) " +
                            " insert into [Re_Sealant_Quotation_Tbl] (ID,CIN,SID,QNO,QDATE,PREPAREDBY,NOTEDBY)values(@id,@cin,@sid,@qno,@qdate,'','')";
                        using (SqlCommand sqlcmd = new SqlCommand(str, sqlcon))
                        {
                            sqlcon.Open();
                            sqlcmd.Parameters.AddWithValue("@cin", cin);
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "myitem")
            {
                ...
                Session["SID"] = sid;
                Response.Redirect("~/AFTERSALESPROJ/resealant_item.aspx");
            }
            else if (e.CommandName == "myreport")
            {
                int rowindex = ((GridViewRow)((LinkButton)e.CommandSource).NamingContainer).RowIndex;
                GridViewRow row = GridView1.Rows[rowindex];
                Session["re_sealantdate"] = ((Label)row.FindControl("lblqdate")).Text;
                Session["re_sealantqno"] = ((Label)row.FindControl("lblqno")).Text;
                Session["re_sealantid"] = ((Label)row.FindControl("lblid")).Text;
                Session["re_sealant_report_sender"] = "re_sealant_page";
        ...
        private void deleteme(string id)
        {
            try
            {
                ...
                {
                    sqlcon.Open();
                    string str = "delete from Re_Sealant_Quotation_Tbl where id = @id";
                    using (SqlCommand sqlcmd = new SqlCommand(str, sqlcon))
                    {
        ...
        private void updateme(string id, string qno, string qdate)
        {
            try
            {
                ...
                {
                    sqlcon.Open();
                    string str = "select * from Re_Sealant_Quotation_Tbl where qno = @qno and not id = @id";
                    using (SqlCommand sqlcmd = new SqlCommand(str, sqlcon))
                    {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand("update Re_Sealant_Quotation_Tbl set qno=@qno,qdate=@qdate where id=@id", sqlcon))
                        {
